...
import {IPlanetProperties} from "../store/threejs/planetObjectsTypes";
...
export interface IPlotProperties {
    ...
    radius: number
    scale: number
    actions: Record<'isUserPlots' | 'isMyPlot' | 'isUserGrid' | 'isPlotGrid' | 'isPlots' | 'isGrid' | 'isDashboard' | 'isDashboardUser', boolean>
    events: {
...
interface IScene {
    ...
    audio?: {
        actions?: Record<'soundVolume', number>
    }
    environment?: {
        source: string
    }
